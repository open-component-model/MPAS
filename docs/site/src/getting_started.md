...
```
***TODO:
> The deployed helmRepository does not have the secret set that would permit pulling the helmcharts. So we need to add a step for the user to make the chart public until we address this shortcoming.
> The deployed podinfo deployment doesn't have the pullImageSecret. In order to solve this, the user should update the config.cue to add a serviceAccountName. The name should be the one generated by target.
